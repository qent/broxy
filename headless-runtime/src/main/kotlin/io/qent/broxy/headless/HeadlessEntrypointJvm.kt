...
fun runStdioProxy(
    ...
    runCatching {
        ...
        val downstreams = buildDownstreams(headlessConfig.config, logger, authStateStore, timeouts)

        val syncBridge = SdkSyncBridge(logger)
        val proxy = createProxy(headlessConfig.config, downstreams, logger, syncBridge)
...
private fun createProxy(
    ...
    ProxyMcpServer(
        downstreams = downstreams,
        logger = logger,
        onCapabilitiesUpdated = { syncBridge.onCapabilitiesUpdated() },
