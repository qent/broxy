...
- `JsonConfigurationRepository` reads/writes `mcp.json` and `preset_*.json`, validates transports,
  ...
  `capabilitiesRefreshIntervalSeconds`, `inboundHttpPort`, `defaultPresetId`.
  UI-only settings (for example `showTrayIcon`) live in `ui.json` and are loaded by the ui-adapter.
