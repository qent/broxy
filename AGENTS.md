...
Примеры команд (используйте то, что применимо к изменённым модулям):

```bash
# Полная сборка (включает тесты через задачу check)
./gradlew clean build

# Запуск всех тестов во всех модулях, где они есть
./gradlew test

# Таргетно: юнит‑тесты core для JVM
./gradlew :core:jvmTest

# Запуск известных тестовых задач по модулям
