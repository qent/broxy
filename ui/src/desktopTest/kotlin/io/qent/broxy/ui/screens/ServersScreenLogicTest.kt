...
class ServersScreenLogicTest {
    @Test
    fun `shouldShowServersSearchField returns true when servers list has items`() {
        assertEquals(
            true,
            shouldShowServersSearchField(
                serverCount = 1,
                importedServerCount = 0,
            ),
        )
    }

    @Test
    fun `shouldShowServersSearchField returns true when import list has items`() {
        assertEquals(
            true,
            shouldShowServersSearchField(
                serverCount = 0,
                importedServerCount = 2,
            ),
        )
    }

    @Test
    fun `shouldShowServersSearchField returns false when both lists are empty`() {
        assertEquals(
            false,
            shouldShowServersSearchField(
                serverCount = 0,
