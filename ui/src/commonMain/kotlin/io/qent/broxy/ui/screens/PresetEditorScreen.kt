...
internal fun mergePresetEditorToolRefs(
    existing: List<UiToolRef>,
    selectedEnabled: List<UiToolRef>,
): List<UiToolRef> {
    val selectedKeys = selectedEnabled.map { it.serverId to it.toolName }.toSet()
    val preservedDisabled =
        existing.filter { ref ->
            !ref.enabled && (ref.serverId to ref.toolName) !in selectedKeys
        }
    return (preservedDisabled + selectedEnabled.map { it.copy(enabled = true) })
        .distinctBy { it.serverId to it.toolName }
}

internal fun mergePresetEditorPromptRefs(
    existing: List<UiPromptRef>,
    selectedEnabled: List<UiPromptRef>,
): List<UiPromptRef> {
    val selectedKeys = selectedEnabled.map { it.serverId to it.promptName }.toSet()
    val preservedDisabled =
        existing.filter { ref ->
            !ref.enabled && (ref.serverId to ref.promptName) !in selectedKeys
        }
    return (preservedDisabled + selectedEnabled.map { it.copy(enabled = true) })
        .distinctBy { it.serverId to it.promptName }
}

internal fun mergePresetEditorResourceRefs(
    existing: List<UiResourceRef>,
    selectedEnabled: List<UiResourceRef>,
): List<UiResourceRef> {
    val selectedKeys = selectedEnabled.map { it.serverId to it.resourceKey }.toSet()
    val preservedDisabled =
        existing.filter { ref ->
            !ref.enabled && (ref.serverId to ref.resourceKey) !in selectedKeys
        }
    return (preservedDisabled + selectedEnabled.map { it.copy(enabled = true) })
        .distinctBy { it.serverId to it.resourceKey }
}

internal fun autoFillPresetNameFromServerSelection(
    currentName: String,
    selectedServerName: String,
    isCreateMode: Boolean,
    selectedWholeServer: Boolean,
): String {
    if (!isCreateMode || !selectedWholeServer || currentName.isNotBlank()) {
        return currentName
    }
    val serverName = selectedServerName.trim()
    return if (serverName.isBlank()) currentName else serverName
}
