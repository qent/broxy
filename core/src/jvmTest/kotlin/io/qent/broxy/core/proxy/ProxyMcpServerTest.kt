...
class ProxyMcpServerTest {
    ...
    @org.junit.Test
    fun applyPresetNotifiesCapabilitiesUpdatedWhenUsingCachedCapabilities() =
        runTest {
            val serverA =
                FakeServerConnection(
                    serverId = "alpha",
            ...
            val proxy =
                ProxyMcpServer(
                    ...
                    logger = noopLogger,
                    onCapabilitiesUpdated = { updates.incrementAndGet() },
                )
            val initialPreset =
                Preset(
                    id = "initial",
            ...
            assertEquals(listOf("beta_translate"), proxy.capabilities.tools.map { it.name })
        }
