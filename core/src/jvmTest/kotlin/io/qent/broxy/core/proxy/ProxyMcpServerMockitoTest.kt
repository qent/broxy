...
class ProxyMcpServerMockitoTest {
    ...
    @Test
    fun refreshServerCapabilities_forces_downstream_refresh() {
        val s1: McpServerConnection = mock()
        whenever(s1.serverId).thenReturn("s1")
        whenever(s1.config).thenReturn(cfg("s1"))
        runBlocking { whenever(s1.getCapabilities(any())).thenReturn(Result.success(ServerCapabilities())) }

        val proxy = ProxyMcpServer(listOf(s1))
        val preset = Preset(id = "p", name = "name", tools = listOf(ToolReference("s1", "tool", true)))
        proxy.start(preset, TransportConfig.HttpTransport("http://0.0.0.0:0/mcp"))

        runBlocking { proxy.refreshServerCapabilities("s1") }

        runBlocking { verify(s1).getCapabilities(true) }
    }
}
