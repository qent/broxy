...
class ConfigurationManagerTest {
    ...
    @Test
    fun upsertServer_inserts_new_server_at_beginning_when_requested() {
        val repo = InMemoryRepo()
        val manager = ConfigurationManager(repo, ConfigTestLogger)
        val config = McpServersConfig(servers = listOf(testServer("s1"), testServer("s2")))

        val result = manager.upsertServer(config, testServer("new"), insertAtBeginning = true)

        assertTrue(result.isSuccess)
        assertEquals(listOf("new", "s1", "s2"), result.getOrThrow().servers.map { it.id })
        assertEquals(listOf("new", "s1", "s2"), repo.savedConfig?.servers?.map { it.id })
    }
