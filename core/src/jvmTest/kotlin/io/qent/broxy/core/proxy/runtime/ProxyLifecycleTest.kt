...
class ProxyLifecycleTest {
    ...
    @Test
    fun restart_uses_current_config_and_inbound() {
        val controller = FakeProxyController()
        val lifecycle = ProxyLifecycle(controller, TestLogger())
        ...
        assertTrue(lifecycle.restart(config = config, preset = nextPreset, inbound = inbound).isSuccess)
    ...
    @Test
    fun restart_fails_when_not_running() {
        val controller = FakeProxyController()
        val lifecycle = ProxyLifecycle(controller, TestLogger())
        ...
        assertTrue(lifecycle.restart(config = config, preset = null, inbound = null).isFailure)
    }
}
