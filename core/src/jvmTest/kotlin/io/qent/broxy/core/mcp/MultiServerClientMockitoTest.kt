...
class MultiServerClientMockitoTest {
    ...
    @Test
    fun fetch_all_caps_and_route_calls_with_mockito() =
        runBlocking {
            val s1: McpServerConnection = mock()
            whenever(s1.serverId).thenReturn("s1")
            whenever(s1.config).thenReturn(cfg("s1"))
            whenever(s1.getCapabilities(false)).thenReturn(
                Result.success(
                    ServerCapabilities(tools = listOf(ToolDescriptor("t1"))),
                ),
            )
            whenever(s1.callTool(any(), any())).thenReturn(
                Result.success(
                    buildJsonObject {
            ...
            assertEquals("s1_t1", tools.first().name)

            val routed = multi.callPrefixedTool("s1_t1")
            assertTrue(routed.isSuccess)

            val unknown = multi.callPrefixedTool("unknown_echo")
