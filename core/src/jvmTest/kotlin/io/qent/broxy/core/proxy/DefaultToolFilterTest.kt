...
class DefaultToolFilterTest {
    ...
    fun filters_and_prefixes_with_mappings() {
        ...
        val names =
            res.capabilities.tools
                .map { it.name }
                .toSet()
        assertEquals(setOf("s1_t1", "s2_t3"), names)
    ...
    fun allows_explicit_prompt_and_resource_selection() {
        ...
        assertTrue("s2_p3" !in res.allowedPrefixedTools)
    }

    @Test
    fun all_enabled_preset_includes_all_capabilities_from_all_servers() {
        val all =
            mapOf(
                "s1" to
                    ServerCapabilities(
                        tools = listOf(ToolDescriptor("t1")),
                        resources = listOf(ResourceDescriptor("r1", uri = "uri1")),
                        prompts = listOf(PromptDescriptor("p1")),
                    ),
                "s2" to
                    ServerCapabilities(
                        tools = emptyList(),
                        resources = listOf(ResourceDescriptor("r2", uri = "uri2")),
                        prompts = listOf(PromptDescriptor("p2")),
                    ),
                "s3" to
                    ServerCapabilities(
        ...
        val res = DefaultToolFilter().filter(all, Preset.allEnabled())

        val toolNames =
            res.capabilities.tools
                .map { it.name }
                .toSet()
        assertEquals(setOf("s1_t1", "s3_t3"), toolNames)
        assertEquals(setOf("s1_t1", "s3_t3"), res.allowedPrefixedTools)
        val promptNames =
            res.capabilities.prompts
                .map { it.name }
                .toSet()
        ...
        val resourceKeys =
            res.capabilities.resources
                .map { it.uri ?: it.name }
                .toSet()
        assertEquals(setOf("uri1", "uri2"), resourceKeys)
        assertEquals("s1", res.promptServerByName["p1"])
        assertEquals("s2", res.promptServerByName["p2"])
        assertEquals("s1", res.resourceServerByUri["uri1"])
        assertEquals("s2", res.resourceServerByUri["uri2"])
    }

    @Test
    fun strips_registry_namespace_prefix_in_prefixed_tool_names() {
        val registryServerId = "io.qent.broxy/time"
        val all =
            mapOf(
                registryServerId to
                    ServerCapabilities(
                        tools = listOf(ToolDescriptor("now")),
                    ),
            )
        val preset =
            Preset(
                ...
                tools =
                    listOf(
                        ToolReference(registryServerId, "now", enabled = true),
                    ),
            )
        ...
        assertEquals(setOf("time_now"), toolNames)
        assertEquals(setOf("time_now"), res.allowedPrefixedTools)
    }
}
