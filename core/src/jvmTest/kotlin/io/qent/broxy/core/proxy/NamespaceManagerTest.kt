...
class NamespaceManagerTest {
    ...
    fun parse_invalid_format_throws() {
        ...
        assertFailsWith<IllegalArgumentException> { ns.parsePrefixedToolName("_bad") }
        assertFailsWith<IllegalArgumentException> { ns.parsePrefixedToolName("bad_") }
    }

    @Test
    fun prefix_tool_name_strips_registry_namespace_prefix() {
        val prefixed = ns.prefixToolName("io.qent.broxy/context7", "search")
        assertEquals("context7_search", prefixed)
        val (sid, tool) = ns.parsePrefixedToolName(prefixed)
        assertEquals("context7", sid)
        assertEquals("search", tool)
    }
    ...
    fun prefix_all_capabilities_prefixes_tools_and_keeps_others() {
        ...
        assertEquals(setOf("s1_t1", "s2_t2"), toolNames)
        // ensure descriptions preserved for tools
        val d1 = prefixed.tools.first { it.name == "s1_t1" }.description
        assertEquals("d1", d1)
