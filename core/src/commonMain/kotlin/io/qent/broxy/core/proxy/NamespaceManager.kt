...
internal const val REGISTRY_NAMESPACE_PREFIX = "io.qent.broxy/"

internal fun isSafeNamespaceServerId(serverId: String): Boolean = !serverId.contains(NAMESPACE_SEPARATOR)

internal fun toFacadeNamespaceServerId(serverId: String): String {
    val trimmed = serverId.trim()
    if (trimmed.isEmpty()) {
        return trimmed
    }
    val normalized = trimmed.removePrefix(REGISTRY_NAMESPACE_PREFIX)
...
class DefaultNamespaceManager : NamespaceManager {
    override fun prefixToolName(
        serverId: String,
        toolName: String,
    ): String = "${toFacadeNamespaceServerId(serverId)}${NAMESPACE_SEPARATOR}$toolName"
