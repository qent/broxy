...
/**
 * Executes [block] while holding [OAuthState.mutex].
 */
suspend inline fun <T> OAuthState.withLock(block: OAuthState.() -> T): T = mutex.withLock { block() }

/**
 * Non-locking read helper for best-effort token access.
 */
fun OAuthState.peekAccessToken(): String? = token?.accessToken

/**
 * Non-locking read helper for best-effort timeout access.
 */
fun OAuthState.peekAuthorizationTimeoutMillis(): Long? = authorizationTimeoutMillis
