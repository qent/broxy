...
class ProxyMcpServer(
    ...
    /** Returns the current downstream connection list used for dispatching. */
    fun downstreamsSnapshot(): List<McpServerConnection> = downstreams.toList()

    /** Applies a new preset at runtime and re-filters cached capabilities. */
    fun applyPreset(preset: Preset) {
        currentPreset = preset
        val reapplyResult =
            runCatching { runBlocking { stateStore.reapplyPreset(preset) } }
        ...
        if (!hasCached && downstreams.isNotEmpty()) {
            ...
            val refreshResult = runCatching { runBlocking { refreshFilteredCapabilities() } }
    ...
    /**
     * Replaces the raw downstream capabilities snapshot and re-applies the current preset without
     * fetching capabilities from downstream connections again.
     */
    fun setCapabilitiesSnapshot(all: Map<String, ServerCapabilities>) {
        val refreshResult =
            runCatching {
                runBlocking {
                    val refreshId = stateStore.nextRefreshSequence()
                    stateStore.applyFullRefresh(refreshId, all, currentPreset)
                }
            }
        if (refreshResult.isFailure) {
            logger.error("Failed to apply external capabilities snapshot", refreshResult.exceptionOrNull())
            return
        }
