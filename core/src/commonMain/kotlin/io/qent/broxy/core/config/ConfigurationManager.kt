...
class ConfigurationManager(
            ...
            if (validation.isFailure) {
                ...
                saveConfig(
                    mutateServers(config) { servers ->
                        ...
                        when {
                            idx >= 0 -> servers[idx] = server
                            insertAtBeginning -> servers.add(0, server)
                            else -> servers += server
                        }
                    },
                )
    ...
    fun reorderServers(
        ...
        runCatching {
            val reordered = reorderByIds(config.servers, orderedServerIds) { it.id }
            if (reordered == config.servers) {
                config
            } else {
                val updated = config.copy(servers = reordered)
                repository.saveMcpConfig(updated)
                updated
            }
        }.onFailure {
            logger.warn("Failed to reorder servers: ${it.message}", it)
        }
    ...
    fun reorderPresets(orderedPresetIds: List<String>): Result<List<Preset>> =
        runCatching {
            val existing = repository.listPresets()
            val reordered = reorderByIds(existing, orderedPresetIds) { it.id }
            if (reordered == existing) {
                existing
            } else {
                val updated =
                    reordered.mapIndexed { index, preset ->
                        preset.copy(orderIndex = index)
                    }
                updated.forEach { repository.savePreset(it) }
                updated
            }
        }.onFailure { logger.warn("Failed to reorder presets: ${it.message}", it) }
    ...
    inner class SettingsManager {
        ...
        fun updateMcpFilePath(
            ...
            saveConfig(
                config.copy(mcpFilePath = path.trim()),
            )
        ...
        val errorMessage =
            when {
                orderedIds.size != items.size ->
                    "Invalid order size: expected ${items.size}, got ${orderedIds.size}"
                orderedIds.toSet().size != orderedIds.size -> "Invalid order: duplicate ids"
                else ->
                    orderedIds
                        .firstOrNull { it !in byId }
                        ?.let { unknownId -> "Invalid order: unknown id '$unknownId'" }
            }
        if (errorMessage != null) throw ConfigurationException(errorMessage)
        return orderedIds.map { id -> byId.getValue(id) }
    }
