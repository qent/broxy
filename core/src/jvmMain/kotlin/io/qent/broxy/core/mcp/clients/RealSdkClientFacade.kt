...
class RealSdkClientFacade(
    ...
    private class CapabilitySupportCache {
        @Volatile
        ...
        fun cachedSupport(capability: Capability): Boolean? =
            when (capability) {
    ...
    private val supportCache = CapabilitySupportCache()
    ...
    private fun isCapabilitySupported(capability: Capability): Boolean {
        val cached = supportCache.cachedSupport(capability)
        if (cached != null) {
            return cached
        }
        ...
        supportCache.recordSupport(capability, supported)
        if (!supported) {
    ...
    private fun recordUnsupportedFromError(
        ...
        val matches = message?.startsWith("Server does not support ${capability.label}") == true
        if (matches) {
            val changed = supportCache.recordSupport(capability, false)
            if (changed) {
                logger.info("Skipping ${capability.listOperation}: server does not support ${capability.label}.")
            }
        }
