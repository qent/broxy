...
internal fun syncSdkServer(
    ...
    resetServerRegistrations(server)
    ...
    val promptsToAdd = buildPromptRegistrations(capabilities, backend, logger, json)
    if (promptsToAdd.isNotEmpty()) {
        server.addPrompts(promptsToAdd)
    }

    val resourcesToAdd = buildResourceRegistrations(capabilities, backend, logger)
    if (resourcesToAdd.isNotEmpty()) {
        server.addResources(resourcesToAdd)
    }
}

private fun resetServerRegistrations(server: Server) {
