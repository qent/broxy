...
internal class ProxyRuntimeLifecycle(
        ...
        val proxy =
            ProxyMcpServer(
                ...
                onCapabilitiesUpdated = { capabilities ->
                    val running = state as? RuntimeState.Running
                    if (running != null && !running.proxy.adapterMode) {
                        running.inbound.refreshCapabilities()
                    }
    ...
    fun updateAdapterMode(enabled: Boolean) {
        ...
        proxy.adapterMode = enabled
        runCatching { runBlocking { proxy.refreshFilteredCapabilities() } }
            .onFailure { logger.warn("Failed to refresh capabilities after adapter mode change", it) }
        refreshInboundCapabilities(force = true)
    }
    ...
    fun refreshInboundCapabilities(force: Boolean = false): Result<Unit>? {
        ...
        val result =
            when {
                running == null -> null
                !force && running.proxy.adapterMode -> Result.success(Unit)
                else -> running.inbound.refreshCapabilities()
            }
