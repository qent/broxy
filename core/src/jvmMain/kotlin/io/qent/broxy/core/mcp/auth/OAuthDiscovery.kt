...
internal suspend fun fetchAuthorizationServerMetadataByUrl(
    client: HttpClient,
    metadataUrl: String,
    logger: Logger,
    json: Json,
): AuthorizationServerMetadata {
    logger.debug("OAuth fetching authorization server metadata from explicit URL $metadataUrl")
    val response =
        client.get(metadataUrl) {
            headers { append(HttpHeaders.Accept, ContentType.Application.Json.toString()) }
        }
    if (!response.status.isSuccess()) {
        error("Failed to fetch OAuth authorization server metadata from $metadataUrl: HTTP ${response.status.value}")
    }
    val body = response.bodyAsText()
    ...
    if (!hasEndpoints) {
        error("OAuth authorization server metadata at $metadataUrl is missing required endpoints")
    }
    return metadata
}
