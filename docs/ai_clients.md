...
Describe AI client connector architecture, config mutation rules, import flow, and per-client format notes.

## When to read

- When adding/changing AI client connectors.
- When changing connect/disconnect behavior or import scanning logic.
- When updating per-client config path or entry format.
...
- `ui-adapter/src/jvmMain/kotlin/io/qent/broxy/ui/adapter/clients/connectors/*`
- `ui-adapter/src/jvmMain/kotlin/io/qent/broxy/ui/adapter/clients/formats/*`

## Behavior contract

Connectors may only add/update/remove the Broxy MCP entry and must preserve all unrelated client config entries.
...
- Search on MCP screen applies to both the main server list and import cards, and the search field is hidden
...
Each tab also shows copy icon controls to the right of the JSON text with the same color as the snippet text.
Clicking an icon copies a transport-specific value (not the full JSON block):

- Copy icon glyph is compact; the hover background and click target keep the same size.

- HTTP: one icon, copies `http://localhost:{port}/mcp`
- STDIO: two icons (stacked vertically):
  - top icon copies `/Applications/broxy.app/Contents/MacOS/broxy`
  - bottom icon copies `--stdio-proxy`
- SSE: one icon, copies `http://localhost:{port}/sse`

After each copy action, the UI shows a confirmation toast.
