...
The Clients screen ends with three static cards that show copyable JSON snippets for universal
connections. HTTP and SSE examples use the current inbound HTTP port from Settings.
...
- Most clients use a `url` entry; Claude uses a local STDIO command entry instead.
