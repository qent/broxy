...
Define the canonical facade contract between inbound transports, MCP SDK server sync, and proxy routing.

## When to read

- When changing `tools/list`/`tools/call` exposure logic.
- When changing namespace, adapter mode, fallback tools, or preset management surface.
- When changing re-sync behavior for running sessions.
...
Tool namespace is canonicalized as `serverId_toolName`. Preset/adapter/management precedence in
`syncSdkServer(...)` defines the MCP surface seen by inbound sessions.
...
- `POST .../mcp/{presetId}` does the same, but the created session is pinned to the preset from the URL.
...
- `InboundStreamableHttpRegistry` stores `sessionId -> ServerSession` plus the route binding for that session.
- If a client reuses a `sessionId` through another route or another preset binding, Broxy returns `409 Conflict`.
- Unknown preset ids on `.../{presetId}` return `404 Not Found`.
...
- `GET /sse/{presetId}` opens an SSE stream pinned to the preset from the URL and advertises
  `/sse/{presetId}?sessionId=...`.
- `POST /sse?sessionId=...` forwards MCP messages into the SSE session.
- `POST /sse/{presetId}?sessionId=...` forwards MCP messages into the matching preset-pinned SSE session.
...
- each preset-management tool is published with a non-null `outputSchema`;
- preset-management tool responses include `structuredContent` only for successful calls;
  error calls return `isError=true` with text content and no `structuredContent`.
...
Inbound (STDIO or Streamable HTTP) does not restart.

- sessions opened through the default routes (`/mcp`, `/sse`) re-sync and start serving the new active preset;
- sessions opened through preset-pinned routes (`/mcp/{presetId}`, `/sse/{presetId}`) keep serving their pinned preset
  and ignore global preset switching.
...
For preset-pinned HTTP/SSE sessions Broxy creates lightweight additional `ProxyMcpServer` instances
that reuse the same downstream connections and raw capability snapshots, but keep their own preset.
