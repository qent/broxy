...
Document inbound transport behavior and session semantics for STDIO, Streamable HTTP, and SSE.

## When to read

- When changing HTTP/SSE endpoints or status code behavior.
- When changing session binding, timeout handling, or preset-pinned routes.
- When changing CLI/UI inbound startup behavior.
...
- `core/src/jvmMain/kotlin/io/qent/broxy/core/proxy/inbound/InboundRegistries.kt`
- `core/src/jvmMain/kotlin/io/qent/broxy/core/proxy/inbound/InboundSessionCleanup.kt`

## Behavior contract

Inbound default routes (`/mcp`, `/sse`) track the active preset; preset-pinned routes keep the preset
resolved at session creation. Session rebinding across route/preset boundaries is rejected with `409 Conflict`.
...
- preset selection order:
    ...
    2) `defaultPresetId` from `config.json`,
...
### Streamable HTTP endpoints: default and preset-pinned

Broxy exposes two Streamable HTTP route shapes on the configured inbound path:

- default route: `POST /mcp` (or the configured base path)
- preset-pinned route: `POST /mcp/{presetId}`
...
1) `POST`:
    ...
    - if the route contains `{presetId}`, the session is pinned to that preset snapshot for its whole lifetime;
...
- unknown preset id on `.../{presetId}` -> `404 Not Found`
- reusing an existing `sessionId` through another route or another preset binding -> `409 Conflict`

Requests use `requestTimeoutSeconds` from configuration (default 60s if not overridden).

### Session binding matrix

| Route | Preset binding | Reacts to global preset switch | 404 behavior | 409 behavior |
| --- | --- | --- | --- | --- |
| `/mcp` | active preset | yes | n/a | session reused through another route/preset |
| `/mcp/{presetId}` | pinned to `{presetId}` | no | unknown preset id | session reused through another route/preset |
| `/sse` | active preset | yes | n/a | session reused through another route/preset |
| `/sse/{presetId}` | pinned to `{presetId}` | no | unknown preset id | session reused through another route/preset |
...
- each session also stores its route binding (`/mcp` vs `/mcp/{presetId}`) and its bound proxy view;
...
- `sessionId` is passed as a query parameter (`/sse?sessionId=...` or `/sse/{presetId}?sessionId=...`).
- each session also stores its route binding (`/sse` vs `/sse/{presetId}`) and its bound proxy view;
...
2) `GET /sse/{presetId}`:
    - opens an SSE stream pinned to the preset from the URL;
    - emits an `endpoint` event pointing clients at `/sse/{presetId}?sessionId=...`.
3) `POST /sse?sessionId=...`:
    - expects `Content-Type: application/json`;
    - forwards the MCP JSON-RPC message into the active SSE session.
4) `POST /sse/{presetId}?sessionId=...`:
    - forwards MCP JSON-RPC into the active preset-pinned SSE session;
    - rejects `sessionId` values created under another route or preset with `409 Conflict`.
