...
Capture stable public contracts and invariants for core runtime behavior.

## When to read

- Before changing interfaces in `core` public runtime/config/proxy contracts.
- Before refactoring routing, filtering, namespace, or inbound semantics.
...
Entries below are compatibility-sensitive and must be updated together with code/tests/docs when changed.
...
### Preset-management tool result contract

- In `__preset_management__` mode, all six fixed management tools are exposed with non-null `outputSchema`.
- For these tools, `structuredContent` is returned only on successful calls.
- Tool-level failures return `isError=true` with text content and no `structuredContent`.

Files:
- `core/src/jvmMain/kotlin/io/qent/broxy/core/proxy/inbound/PresetManagementToolFactory.kt`
- `core/src/jvmMain/kotlin/io/qent/broxy/core/proxy/inbound/PresetManagementToolSchemas.kt`
