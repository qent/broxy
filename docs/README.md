...
Эта папка содержит подробные описания ключевых подсистем broxy: архитектуры, прокси-фасада, подключений downstream MCP-серверов, пресетов/фильтрации capabilities, конфигурации/хот-релоада, inbound-транспортов (STDIO/HTTP Streamable), а также авторизации и удалённого режима через WebSocket с бэкэндом `broxy.run`.
