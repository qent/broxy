...
Document the allowed `ui-adapter -> core` dependency boundary and guardrail policy.

## When to read

- When adding `io.qent.broxy.core.*` imports in `ui-adapter`.
- When changing boundary-check rules or mapper ownership.
- When refactoring store/runtime wiring across modules.

## Source-of-truth files

- `ui-adapter/build.gradle.kts`
...
`ui-adapter` consumes `core` contracts and constrained runtime wiring only; direct `core` implementation
sprawl outside the allowlist is blocked by `:ui-adapter:checkUiAdapterCoreBoundary`.

This document captures the current `ui-adapter -> core` import inventory, highlights disputed
dependencies, and defines the target boundary enforced by the boundary-check task.
...
- `ui-adapter/src/commonMain/kotlin/io/qent/broxy/ui/adapter/store/AppStore.kt` -> `io.qent.broxy.core.config`
- `ui-adapter/src/commonMain/kotlin/io/qent/broxy/ui/adapter/store/internal/AppStoreIntents.kt` -> `io.qent.broxy.core.config`
- `ui-adapter/src/commonMain/kotlin/io/qent/broxy/ui/adapter/store/internal/StoreConfigGateway.kt` -> `io.qent.broxy.core.config`
- `ui-adapter/src/jvmMain/kotlin/io/qent/broxy/ui/adapter/data/RepositoriesJvm.kt` -> `io.qent.broxy.core.config`
- `ui-adapter/src/jvmMain/kotlin/io/qent/broxy/ui/adapter/services/ToolServiceJvm.kt` -> `io.qent.broxy.core.config`
