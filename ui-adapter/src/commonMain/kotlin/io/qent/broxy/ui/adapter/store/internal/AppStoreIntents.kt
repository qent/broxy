...
internal class AppStoreIntents(
    ...
    override fun reorderServers(serverIds: List<String>) {
        scope.launch {
            ...
            state.updateSnapshot { copy(servers = reordered) }
            publishReady()
            val result =
                withContext(ioDispatcher) {
                    configurationManager.reorderServers(previousConfig.toCore(), serverIds)
                }
            if (result.isFailure) {
                revertServersOnFailure(
                    "reorderServers",
            ...
            val savedConfig = result.getOrNull()
            capabilityRefresher.syncWithServers(savedConfig?.servers ?: reordered.toCore())
            applyServerConfigToProxy(savedConfig?.toUi(), "reorderServers")
            publishReady()
        }
    }
    ...
    override fun reorderPresets(presetIds: List<String>) {
        scope.launch {
            ...
            state.updateSnapshot { copy(presets = reordered) }
            publishReady()
            val result =
                withContext(ioDispatcher) {
                    configurationManager.reorderPresets(presetIds)
                }
            if (result.isFailure) {
                revertPresetsOnFailure(
                    "reorderPresets",
