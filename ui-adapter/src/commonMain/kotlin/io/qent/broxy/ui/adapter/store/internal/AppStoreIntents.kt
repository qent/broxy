...
internal class AppStoreIntents(
    ...
    override fun upsertPreset(draft: UiPresetDraft) {
        scope.launch {
            ...
            if (saveResult.isSuccess && renameId != null) {
                val wasSelected = previousSnapshot.selectedPresetId == renameId
                if (wasSelected) {
                    val configSave =
                        withContext(ioDispatcher) {
            ...
            val shouldRestart =
                saveResult.isSuccess &&
                    (
                        previousSnapshot.selectedPresetId == preset.id ||
                            (renameId != null && previousSnapshot.selectedPresetId == renameId)
                    )
            publishReady()
    ...
    override fun selectProxyPreset(presetId: String?) {
        scope.launch {
            ...
            if (saveResult.isFailure) {
                val msg = logFailure(logger, "selectProxyPreset", saveResult.exceptionOrNull(), "Failed to save preset selection")
    ...
    override fun updateInboundSsePort(port: Int) {
        scope.launch {
            ...
            state.updateSnapshot { copy(inboundSsePort = clamped, clients = updatedClients) }
            publishReady()
            val result =
                withContext(ioDispatcher) {
                    configurationManager.settings.updateInboundSsePort(previousConfig.toCore(), clamped)
                }
            if (result.isFailure) {
                val msg = logFailure(logger, "updateInboundSsePort", result.exceptionOrNull(), "Failed to update HTTP port")
                state.updateSnapshot { copy(inboundSsePort = previous, clients = previousClients) }
                state.setError(msg)
                publishReady()
                return@launch
            }
    ...
    override fun connectAiClient(clientId: String) {
        scope.launch {
            ...
            val request = AiClientConnectionRequest(httpEndpoint = httpEndpointFor(state.snapshot.inboundSsePort))
            val result =
                withContext(ioDispatcher) {
    ...
    private suspend fun refreshAiClients() {
        ...
        val clients = buildAiClients(state.snapshot.inboundSsePort)
