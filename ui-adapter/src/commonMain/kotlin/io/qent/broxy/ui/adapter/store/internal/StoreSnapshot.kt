...
internal data class StoreSnapshot(
    ...
    val toastMessageId: Long = 0,
    val servers: List<UiMcpServerConfig> = emptyList(),
    val importedServerGroups: List<ImportedClientGroup> = emptyList(),
    val catalogServerEntries: List<CatalogServerEntry> = emptyList(),
    ...
    val pendingCatalogInstallSession: CatalogInstallSession? = null,
    val pendingCatalogInstallRequestId: Long = 0L,
    val pendingCatalogInstalledServerId: String? = null,
    val pendingCatalogInstalledServerRequestId: Long = 0L,
)
...
internal fun StoreSnapshot.withPresets(newPresets: List<UiPreset>): StoreSnapshot {
    ...
    if (newPresets.isEmpty()) {
        return copy(
            presets = emptyList(),
            defaultPresetId = defaultPresetId?.takeIf { it in builtinIds },
    ...
    return copy(
        presets = newPresets,
        defaultPresetId = defaultPresetId?.takeIf { it in validIds },
        activeProxyPresetId = activeProxyPresetId,
    )
}
...
internal fun StoreSnapshot.toUiState(
    ...
    val registryMetadata = ServerIconResolver.registryMetadata(catalogServerEntries)
    ...
    val uiServers =
        servers.map { server ->
            ...
            UiServer(
                ...
                externalUrl = matchedMetadata?.externalUrl,
                matchedDescription = matchedMetadata?.description,
            )
        }
    val installedServerIds = servers.map { it.id }.toSet()
    val catalogItems =
        CatalogInstallPlanner.toServerItems(
            entries = catalogServerEntries,
            installedServerIds = installedServerIds,
        )
    return UIState.Ready(
        servers = uiServers,
        importedServerGroups =
            importedServerGroups.map { group ->
                UiImportedServerGroup(
                    clientId = group.clientId,
                    clientName = group.clientName,
                    clientIconId = group.clientIconId,
                    servers =
                        group.servers.map { candidate ->
                            UiImportedServer(
                                sourceServerId = candidate.sourceServerId,
                                name = candidate.config.name,
                                transportLabel = candidate.config.transport.transportLabel(),
                                icon = ServerIconResolver.resolve(candidate.config, registryIconUrls),
                            )
                        },
                )
            },
        ...
        pendingImportedServerCreateRequestId = pendingImportedServerCreateRequestId,
        catalogServers = catalogItems,
        catalogLoading = catalogLoading,
        catalogErrorMessage = catalogErrorMessage,
        catalogUpdatedAtEpochMillis = catalogUpdatedAtEpochMillis,
        pendingCatalogInstallSession = pendingCatalogInstallSession,
        pendingCatalogInstallRequestId = pendingCatalogInstallRequestId,
        pendingCatalogInstalledServerId = pendingCatalogInstalledServerId,
        pendingCatalogInstalledServerRequestId = pendingCatalogInstalledServerRequestId,
