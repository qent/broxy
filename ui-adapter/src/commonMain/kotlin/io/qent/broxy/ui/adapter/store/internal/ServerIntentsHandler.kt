...
internal class ServerIntentsHandler(
    ...
    fun removeServer(id: String) {
        context.scope.launch {
            ...
            val previousPopupQueue = context.state.snapshot.authorizationPopupQueue
            val previousConfig = context.state.snapshotConfig()
            val removedIconPath = previousServers.firstOrNull { it.id == id }?.iconPath
            val updated = previousServers.filterNot { it.id == id }
            val popupState = removeAuthorizationPopupsForServer(previousPopup, previousPopupQueue, id)
            context.state.updateSnapshot {
                copy(
                    servers = updated,
                    authorizationPopup = popupState.active,
                    authorizationPopupQueue = popupState.queue,
                )
            }
            cancelAuthorizationPopups(id, popupState.removed, "removeServer")
            val result =
                withContext(context.ioDispatcher) {
    ...
    fun toggleServer(
        ...
        context.scope.launch {
            ...
            val popupState =
                if (!enabled) {
                    removeAuthorizationPopupsForServer(previousPopup, previousPopupQueue, id)
                } else {
                    AuthorizationPopupState(
                        active = previousPopup,
                        queue = previousPopupQueue,
                        removed = emptyList(),
                    )
                }
            context.state.updateSnapshot {
            ...
            if (!enabled) {
                cancelAuthorizationPopups(id, popupState.removed, "toggleServer")
    ...
    fun cancelAuthorization(serverId: String) {
        ...
        val popupState =
            removeAuthorizationPopupsForServer(
                activePopup = snapshot.authorizationPopup,
                queuedPopups = snapshot.authorizationPopupQueue,
                serverId = serverId,
            )
        if (popupState.removed.isNotEmpty()) {
            context.state.updateSnapshot {
                copy(
                    authorizationPopup = popupState.active,
                    authorizationPopupQueue = popupState.queue,
        ...
        cancelAuthorizationPopups(serverId, popupState.removed, "cancelAuthorization")
    ...
    fun openAuthorizationInBrowser(
        ...
        context.scope.launch {
            ...
            if (popup.status != UiAuthorizationPopupStatus.AwaitingBrowserPermission) {
                return@launch
            }
            context.state.updateSnapshot {
                val activePopup = authorizationPopup
                if (activePopup?.serverId == serverId && activePopup.status == UiAuthorizationPopupStatus.AwaitingBrowserPermission) {
                    copy(authorizationPopup = activePopup.copy(status = UiAuthorizationPopupStatus.Pending))
                } else {
                    this
                }
            }
    ...
    fun dismissAuthorizationPopup(serverId: String) {
        context.scope.launch {
            val snapshot = context.state.snapshot
            val activePopup = snapshot.authorizationPopup
            val queuedPopups = snapshot.authorizationPopupQueue
            val nextState =
                when {
                    activePopup?.serverId == serverId -> {
                        val (nextPopup, remainingQueue) = dequeueNextPopup(queuedPopups)
                        AuthorizationPopupState(
                            active = nextPopup,
                            queue = remainingQueue,
                            removed = listOf(activePopup),
                        )
                    }

                    queuedPopups.any { it.serverId == serverId } ->
                        AuthorizationPopupState(
                            active = activePopup,
                            queue = queuedPopups.filterNot { it.serverId == serverId },
                            removed = queuedPopups.filter { it.serverId == serverId },
                        )
            ...
            context.state.updateSnapshot {
                copy(
                    authorizationPopup = nextState.active,
                    authorizationPopupQueue = nextState.queue,
                )
            }
            context.publishReady()
        }
    ...
    private fun removeAuthorizationPopupsForServer(
        activePopup: UiAuthorizationPopup?,
        queuedPopups: List<UiAuthorizationPopup>,
        serverId: String,
    ): AuthorizationPopupState {
        val removedFromQueue = queuedPopups.filter { it.serverId == serverId }
        val deduplicatedQueue = queuedPopups.filterNot { it.serverId == serverId }
        return if (activePopup?.serverId == serverId) {
            val (nextPopup, remainingQueue) = dequeueNextPopup(deduplicatedQueue)
            AuthorizationPopupState(
                active = nextPopup,
                queue = remainingQueue,
                removed = listOf(activePopup) + removedFromQueue,
            )
        } else {
            AuthorizationPopupState(
                active = activePopup,
                queue = deduplicatedQueue,
    ...
    private fun cancelAuthorizationPopups(
        serverId: String,
        popups: List<UiAuthorizationPopup>,
        operation: String,
    ) {
        popups.forEach { popup ->
            context.scope.launch {
                ...
                cancelResult.onFailure {
                    logFailure(
                        context.logger,
                        "$operation(id=$serverId)/cancelAuthorization",
                        it,
    ...
    private data class AuthorizationPopupState(
        val active: UiAuthorizationPopup?,
        val queue: List<UiAuthorizationPopup>,
        val removed: List<UiAuthorizationPopup>,
    )
}
