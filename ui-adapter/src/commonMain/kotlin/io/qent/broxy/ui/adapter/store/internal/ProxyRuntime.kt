...
internal class ProxyRuntime(
    ...
    suspend fun ensureInboundRunning(
        ...
        val presetId =
            presetIdOverride
                ?: state.snapshot.activeProxyPresetId
                ?: state.snapshot.defaultPresetId
        ...
        if (shouldStartOrRestart) {
            ...
            if (preset.isFailure) {
                ...
                publishReady()
                return Result.failure(preset.exceptionOrNull() ?: IllegalStateException("Failed to load preset"))
            }
            ...
            if (result.isSuccess) {
                ...
                remoteConnector.onProxyRunningChanged(false)
                publishReady()
                return Result.failure(result.exceptionOrNull() ?: IllegalStateException("Failed to start HTTP server"))
            }
        }

        if (!isRunning || (!presetChanged && !forceReloadPreset)) {
            return Result.success(Unit)
        }
        ...
        if (applyResult.isSuccess) {
            ...
            return Result.failure(applyResult.exceptionOrNull() ?: IllegalStateException("Failed to apply preset"))
        }
    }
