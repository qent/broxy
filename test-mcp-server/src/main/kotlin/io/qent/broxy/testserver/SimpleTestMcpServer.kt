...
class SimpleTestMcpServer(
    ...
    private fun buildServer(profile: ModeProfile): Server {
        val supportsTools = options.capabilities.contains(ServerCliOptions.Capability.TOOLS)
        val supportsPrompts = options.capabilities.contains(ServerCliOptions.Capability.PROMPTS)
        val supportsResources = options.capabilities.contains(ServerCliOptions.Capability.RESOURCES)
        val server =
            Server(
                serverInfo =
                    Implementation(
...
data class ServerCliOptions(
    ...
    val capabilities: Set<Capability> = Capability.entries.toSet(),
) {
    enum class Mode {
        STDIO,
        HTTP_STREAMABLE,
    ...
    companion object {
        ...
        fun parse(args: Array<String>): ServerCliOptions {
            ...
            var capabilities = Capability.entries.toSet()
            ...
            while (index < args.size) {
                when (val arg = args[index]) {
                    ...
                    "--capabilities" -> {
                        val value = requireNextArg(args, index, "--capabilities")
        ...
        private fun parseMode(value: String): Mode =
            when (value.lowercase()) {
                "stdio" -> Mode.STDIO
                "http" -> Mode.HTTP_STREAMABLE
                "sse" -> Mode.HTTP_SSE
                "ws" -> Mode.WS
                else -> error("Unsupported mode '$value'. Use 'stdio', 'http', 'sse', or 'ws'.")
            }

        private fun parseCapabilities(value: String): Set<Capability> {
            val trimmed = value.trim()
            if (trimmed.equals("all", ignoreCase = true)) {
                return Capability.entries.toSet()
            }
            val tokens =
                trimmed
                    .split(",")
            ...
            if (tokens.isEmpty()) {
                error("Capabilities list cannot be empty. Use 'all' or a comma-separated list.")
            }
            return tokens
                .map { token ->
                    when (token) {
                        ...
                        else -> error("Unknown capability '$token'. Use tools, prompts, resources, or all.")
                    }
                }.toSet()
        }
    }
