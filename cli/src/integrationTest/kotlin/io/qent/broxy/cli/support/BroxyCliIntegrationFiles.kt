...
internal object BroxyCliIntegrationFiles {
    fun prepareConfigDir(
        ...
        val commandOverride =
            when (scenario.stdioCommandMode) {
                BroxyCliIntegrationConfig.StdioCommandMode.DIRECT -> null
                BroxyCliIntegrationConfig.StdioCommandMode.OUTAGE_WRAPPER -> createStdioOutageWrapper(dir)
            }
        writeTestServerConfigFiles(
            destinationDir = dir,
    ...
    private fun writeTestServerConfigFiles(
        destinationDir: Path,
        httpServerUrl: String,
        sseServerUrl: String,
        wsServerUrl: String,
        ...
        val command = jsonEscape(commandOverride ?: resolveTestServerCommand())
        val resolved =
            template
        ...
        val root: JsonObject = JSON.parseToJsonElement(resolved).jsonObject
        val mcpServers =
            root["mcpServers"]
                ?: fail("Integration config template '$templateResource' must contain 'mcpServers'")
        val mcpRoot = buildJsonObject { put("mcpServers", mcpServers) }
        val appConfig =
            buildJsonObject {
        ...
        Files.writeString(
            destinationDir.resolve("mcp.json"),
            JSON.encodeToString(JsonObject.serializer(), mcpRoot),
        )
        Files.writeString(
            destinationDir.resolve("config.json"),
            JSON.encodeToString(JsonObject.serializer(), appConfig),
        )
    }
    ...
    private fun createStdioOutageWrapper(dir: Path): String {
        val marker = BroxyCliIntegrationConfig.STDIO_OUTAGE_MARKER_FILE
        val command = resolveTestServerCommand()
        val fileName = if (isWindows()) "stdio-outage-wrapper.bat" else "stdio-outage-wrapper.sh"
        val wrapper = dir.resolve(fileName)
        val script =
            if (isWindows()) {
                """
                @echo off
                set "SCRIPT_DIR=%~dp0"
                set "MARKER=%SCRIPT_DIR%$marker"
                if exist "%MARKER%" (
                  echo Simulated STDIO outage 1>&2
                  exit /b 1
                )
                "$command" %*
                """.trimIndent()
        ...
        return wrapper.toAbsolutePath().pathString
    }
