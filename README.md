...
Поддерживаются транспорты: `stdio`, `http`, `streamable-http`, `websocket`. Формат — плоский (без вложенного объекта `transport`):
...
Примечания:
- Переменные окружения подставляются ТОЛЬКО в секции `env` по шаблону `${VAR}`. Значения в `headers`, `url` и т.п. должны быть уже подставлены.
- Поле `env` применяется к STDIO-серверам (как переменные процесса). Для HTTP/WS оно не используется транспортом напрямую.
...
Редактор открывается как отдельный экран внутри вкладки **Presets** (пункт меню **Presets** остаётся активным во время создания/редактирования).
...
Если нужно запустить Compose UI на macOS напрямую из исходников (без упаковки в `.dmg`), используйте Gradle-задачу `run`:

```bash
./gradlew :ui:run
```

Она компилирует и запускает приложение в текущем терминале, поэтому логирование и вывод остаются под рукой. Убедитесь, что `JAVA_HOME` указывает на JDK 17 или 21, как описано в разделе "Сборка из исходников".
...
Автоматический прогон `./gradlew :cli:integrationTest` собирает shadow JAR, запускает broxy в режимах STDIO и HTTP Streamable и проверяет работу с локальным тестовым MCP сервером `test-mcp-server` (2 инструмента, 2 ресурса, 2 промпта), согласно пресету `preset_test.json`.

- Конфигурационные файлы для теста лежат в `cli/src/integrationTest/resources/integration/mcp.json` и `preset_test.json`.
- STDIO сервер собирается локально (модуль `test-mcp-server`), для `exa-search` требуется доступ в интернет.
- Тесты:
  - строят `broxy-cli-*.jar`;
  - запускают CLI в STDIO режиме и через MCP протокол проверяют список инструментов/промптов/ресурсов, затем вызывают разрешённые пресетом элементы;
  - повторяют проверку для HTTP Streamable входа, убеждаясь, что фильтрация по `preset_test.json` соблюдается, а все инструменты/ресурсы исполняются без ошибок.
...
A: Да. И UI, и CLI автоматически отслеживают изменения файлов `mcp.json` и `preset_*.json` в `--config-dir` и применяют их (с дебаунсом ~300мс).
