...
Автоматический прогон `./gradlew :cli:integrationTest` собирает shadow JAR, запускает broxy в режимах STDIO и HTTP+SSE и проверяет реальную работу с MCP серверами `time` и `exa-search` согласно пресету `preset_test.json`.

- Конфигурационные файлы для теста лежат в `cli/src/integrationTest/resources/integration/mcp.json` и `preset_test.json`.
- Перед запуском убедитесь, что локально установлен STDIO MCP сервер времени (`/opt/homebrew/bin/uvx mcp-server-time --local-timezone=America/New_York`) и доступен интернет для `https://mcp.exa.ai`.
- Тесты:
  - строят `broxy-cli-*.jar`;
  - запускают CLI в STDIO режиме и через MCP протокол проверяют список инструментов/промптов/ресурсов, затем вызывают каждый из них;
