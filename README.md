...
4. Для сетевого HTTP+SSE входа укажите URL (например при подключении внешних клиентов):
```bash
java -jar cli/build/libs/broxy-cli-0.1.0.jar proxy \
  --config-dir ~/.config/broxy \
  --preset-id developer \
...
Автоматический прогон `./gradlew :cli:integrationTest` собирает shadow JAR, запускает broxy в режимах STDIO и HTTP+SSE и проверяет работу с локальным тестовым STDIO MCP сервером `test-mcp-server` (2 инструмента, 2 ресурса, 2 промпта) вместе с HTTP сервером `exa-search`, согласно пресету `preset_test.json`.
...
- Тесты:
  ...
  - повторяют проверку для HTTP+SSE входа, убеждаясь, что фильтрация по `preset_test.json` соблюдается, а все инструменты/ресурсы исполняются без ошибок.
